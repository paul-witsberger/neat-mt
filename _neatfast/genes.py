...
class BaseGene(object):
    ...
    def mutate(self, config):
        for a in self._gene_attributes:
            ...
            ### Paul's custom code ###
            if not 'activation' in a.name or isinstance(self.key, tuple) or (self.key < 0 or self.key > config.num_outputs):
                setattr(self, a.name, a.mutate_value(v, config))
            else:
                # trying to mutate the activation function of an output node - we don't want to let this happen
                pass
