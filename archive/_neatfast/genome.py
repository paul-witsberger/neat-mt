...
class DefaultGenome(object):
    ...
    def create_node(config, node_id):
        ...
        ### Paul's custom code ###
        if node_id >= 0 and node_id < config.num_outputs:
            if len(config.activation_default_out) > 1:
                node.activation = config.activation_default_out[node_id]
            else:
                node.activation = config.activation_default_out[0]
        return node
