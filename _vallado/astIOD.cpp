...
)
{
	...
	printf("mu = %f\np = %f\nmagr1 = %f\nmagr2 = %f\nx1 = %f\nx2 = %f\n",
		mu, p, magr1, magr2, x1, x2);
	for (i = 0; i < 3; i++)
	{
		v1t[i] = (sqrt(mu * p) / magr1) * ((x1 / mu)*r1[i] + rcrv[i]) / magr1;
		v2t[i] = (sqrt(mu * p) / magr2) * ((x2 / mu)*r2[i] + rcrr[i]) / magr2;
		printf("r1[%d]\t\t|\tr2[%d]\t\t|\trcrv[%d]\t\t|\trcrr[%d]\t\t|\tv1t[%d]\t\t|\tv2t[%d]\n",
			i, i, i, i, i, i);
		printf("%f\t|\t%f\t|\t%f\t|\t%f\t|\t%f\t|\t%f\n\n",
			r1[i], r2[i], rcrv[i], rcrr[i], v1t[i], v2t[i]);
	}
}  // lambhodograph
...
void lambertbattin
(
double r1[3], double r2[3], double mu, double v1[3], char dm, char df, int nrev, double dtsec,
double v1t[3], double v2t[3], int& error, FILE *outfile, bool printToFile
)
{
	const double small = 1e-8;
	// const double mu = 398600.4418;  // m3s2
	double rcrossr[3];
	int   i, loops, maxLoops = 4;
	...
	if ((df == 'r') && (nrev > 0))
	{
		...
		loops = 1;
		while ((abs(xn - x) >= small) && (loops <= maxLoops))
		{
			x = xn;
			temp = 1.0 / (2.0*(L - x*x));
			temp1 = sqrt(x);
	...
	else
	{
		...
		loops = 1;
		x = 10.0;  // starting value
		while (((abs(xn - x) >= small) && (loops < maxLoops)) || (loops <= 3))
		{
			if (nrev > 0)
			{
				x = xn;
			...
			if (printToFile)
				fprintf(outfile, " %3i yb %11.6f x %11.6f xn %11.6f dx %11.5e k2 %11.6f b %11.6f u %11.6f\n",
					loops, y, x, xn, xn - x, k2, b, u);
			++loops;
		}  // while
	}

	if ((loops < maxLoops) || true)
	{
		...
		for (i = 0; i < 3; i++)
		{
			v1t[i] = 1.0 / (lam * (1.0 + lam)) * sqrt(mu * (1.0 + x) / (2.0 * s * s * s * (L + x))) * ((r2[i] - r1[i]) + s * pow(1.0 + lam, 2) * (L + x) / (magr1 * (1.0 + x)) * r1[i]);
			v2t[i] = 1.0 / (lam * (1.0 + lam)) * sqrt(mu * (1.0 + x) / (2.0 * s * s * s * (L + x))) * ((r2[i] - r1[i]) - s * pow(1.0 + lam, 2) * (L + x) / (magr2 * (1.0 + x)) * r2[i]);
		}
		if (printToFile) {
			fprintf(outfile, "Loechler v1t: [\t%10f,\t%10f,\t%10f]\n", v1t[0], v1t[1], v1t[2]);
			fprintf(outfile, "Loechler v2t: [\t%10f,\t%10f,\t%10f]\n", v2t[0], v2t[1], v2t[2]);
			fprintf(outfile, "mag(v1t) = %10f\nmag(v2t) = %10f\n", astMath::mag(v1t), astMath::mag(v2t));
		}
	}  // if loops converged < maxLoops

	if (printToFile) {
		if (dtsec < 0.001)
			fprintf(outfile, " \n");
		else {
			fprintf(outfile, "\tnrev\tdm\tloops\tdtsec\t\t\ty\t\t\tf\t\t\tv1t[0]\t\tv1t[1]\t\tv1t[2]\t\tv2t[0]\t\tv2t[1]\t\tv2t[2]\n");
			fprintf(outfile, "x\t%i\t\t%c\t%i\t\t%12.1f\t%8f\t%8f\t%8f\t%10.6f\t%10.6f\t%10.6f\t%10.6f\t%10.6f\t\n",
				nrev, dm, loops, dtsec, y, f, v1t[0], v1t[1], v1t[2], v2t[0], v2t[1], v2t[2]);
		}
	}
}  // lambertbattin
...
double v1[3], double v2[3], int& error, FILE *outfile, bool printToFile
)
{
	const double small = 0.0000001;  // 1e-7
	const int numiter = 60;
	const double mu = 398600.4418;  // m3s2
	double learningRate = 1.0, lrDecrement = 0.9;
	bool doChange = true;
	int lrCtr = 0, lrInterval = 3;

	int loops, ynegktr;
	double vara, y, upper, lower, cosdeltanu, f, g, gdot, xold, xoldcubed, magr1, magr2,
		psiold, psinew, c2new, c3new, dtnew, c2dot, c3dot, dtdpsi, psiold2, psinewPrev;
	...
	{
		loops = 0;
		ynegktr = 1; // y neg ktr -> a counter for how many time y has been negative
		dtnew = -10.0;
		while ((fabs((dtnew - dtsec) / dtsec) >= small) && (loops < numiter) && (ynegktr <= 10))
		{
			if (printToFile)
				fprintf(outfile, "loops = %d\n", loops);
			if (fabs(c2new) > small)
			...
			if ((vara > 0.0) && (y < 0.0))
			{
				...
				while ((y < 0.0) && (ynegktr < 10))
				{
					...
					if (fabs(c2new) > small) {
						...
						y = magr1 + magr2;
				        if (printToFile)
			            	fprintf(outfile, "%3d %10.5f %10.5f %10.5f %7.3f %9.5f %9.5f\n",
			            		loops, psiold, y, xold, dtnew, vara, upper, lower);
					}
					ynegktr++;
				}
				if ((lrCtr % lrInterval == 0) && (learningRate >= 0.5))
					learningRate *= lrDecrement;
				++lrCtr;
				if (printToFile) {
					fprintf(outfile, "upper (after y check) = %f\n", upper);
					fprintf(outfile, "lower (after y check) = %f\n", lower);
					fprintf(outfile, "learningRate (after y check) = %f\n", learningRate);
				}
			}
			...
			if (ynegktr < 10)
			{
				...
				psinew = psiold - (dtnew - dtsec) / dtdpsi * learningRate;
				if ((fabs((psinew - psinewPrev) / psinew) < small * 10) && (loops > 0)) {
					if (printToFile) {
						fprintf(outfile, "lrCtr = %d\n", lrCtr);
						fprintf(outfile, "lrCtr mod lrInterval = %d\n", lrCtr % lrInterval);
					}
					if ((lrCtr % lrInterval == 0) && (learningRate >= 0.5)) {
						learningRate *= lrDecrement;
					}
				...
				if (printToFile) {
					fprintf(outfile, "psiold = %f\n", psiold);
					fprintf(outfile, "psinew = %f\n", psinew);
					fprintf(outfile, "psinewPrev = %f\n", psinewPrev);
					fprintf(outfile, "learningRate = %f\n", learningRate);
					fprintf(outfile, "upper (before change) = %f\n", upper);
					fprintf(outfile, "lower (before change) = %f\n", lower);
					fprintf(outfile, "dt error = %e\n", fabs((dtnew - dtsec) / dtsec));
					fprintf(outfile, "psi error = %e\n", fabs((psinew - psinewPrev) / psinew));
					fprintf(outfile, "lrCtr = %d\n", lrCtr);
				}
				psinewPrev = psinew;
				...
				if (abs(psinew) > upper || psinew < lower)
				{
					...
					if (printToFile)
						fprintf(outfile, "...jk psinew = %f\n", psinew);
				}
				if (printToFile) {
					fprintf(outfile, "upper (after change) = %f\n", upper);
					fprintf(outfile, "lower (after change) = %f\n", lower);
				}
				...
				// if ((dtnew > dtsec) && (loops <= 1))
				// 	dtnew = dtsec - 1.0;
				if (printToFile) {
					fprintf(outfile, "dtnew = %f\n", dtnew);
					fprintf(outfile, "dtsec = %f\n\n", dtsec);
				}
			}
...
)
{
	...
	outfile = fopen("testastIOD.out", "w");
	...
	{
		int numiter = 40;
		lambertuniv(rint, r1tgt, dm, df, 'n', dtsec, v1t, v2t, error, outfile, true);
